...
async def message_button(update, context):
    ...
    prompt = load_prompt(query)
    user_chat_history = "\n\n".join(dialog.list)
    my_message = await send_text(update, context, "Чат GPT 🧠 думает над вариантами ответов...")
...
async def profile_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        my_message = await send_text(update, context, "Чат GPT 🧠 занимается генерацией вашего профиля...")
...
async def opener_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        my_message = await send_text(update, context, "Чат GPT 🧠 занимается генерацией сообщения...")
        answer = await chatgpt.send_question(prompt, user_info)
        await my_message.edit_text(answer)
...
app = ApplicationBuilder().token("6418746314:AAFGnbs1hK2stj3SQ5vnUqnhlQmpGWGALeU").build()
